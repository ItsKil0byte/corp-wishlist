		...
		<div className="w-full min-h-12 px-2 my-2 gap-x-4 flex items-center">
            <div className="w-full h-8 flex justify-center items-center relative">
                <div className={`w-fit h-full flex items-center absolute top-0 left-0 ${!hasBackButton && "hidden"}`}
                     onClick={onBack}
                >
                    <IoIosArrowBack className={"w-8 h-8 fill-main-theme"}/>
                </div>
                <div className={`w-[55%] h-full flex justify-center items-center text-black font-bold truncate ${!hasText && "hidden"}`}>
                    <span className="truncate w-full text-center">
                        {text}
                    </span>
                </div>
                <div className={`w-12 h-full flex justify-center items-center absolute top-0 ${!hasShareButton ? "right-0" : "right-10"} ${!hasEditButton && "hidden"}`}
                     onClick={onEdit}
                >
