  ...
  return (
    <div className="relative w-full aspect-square md:aspect-21/9 glass-strong rounded-[3rem] overflow-hidden shadow-2xl border-white/20 group/collage">
      ...
      <div className="absolute top-[15%] md:top-[10%] right-[5%] w-[50%] md:w-[32%] rotate-2 shadow-2xl rounded-2xl z-20 hover:rotate-0 hover:scale-110 transition-all duration-500 group/item3 cursor-help">
        <div className="bg-[#F2F2F7] dark:bg-gray-800 rounded-2xl border border-white/10 dark:border-white/5 p-4 md:p-6 flex flex-col gap-3 md:gap-4 overflow-hidden h-full min-h-[180px] md:min-h-[240px]">
          {/* Сообщения */}
          <div className="bg-white dark:bg-gray-700 p-3 rounded-2xl rounded-bl-none shadow-sm self-start max-w-[90%] text-xs md:text-sm font-medium text-gray-800 dark:text-gray-100">
            А что подарим?
          </div>
          <div className="bg-white dark:bg-gray-700 p-3 rounded-2xl rounded-bl-none shadow-sm self-start max-w-[90%] text-xs md:text-sm font-medium text-gray-800 dark:text-gray-100 delay-75">
            Может сертификат?
          </div>
          <div className="bg-gray-300 dark:bg-main-theme/20 p-3 rounded-2xl rounded-br-none shadow-sm self-end max-w-[90%] text-xs md:text-sm font-medium text-gray-900 dark:text-main-theme delay-150">
            У него уже есть кружка...
          </div>
          <div className="bg-white dark:bg-gray-700 p-3 rounded-2xl rounded-bl-none shadow-sm self-start max-w-[90%] text-xs md:text-sm font-medium text-gray-800 dark:text-gray-100 delay-300">
            Скиньте номер карты
          </div>
        </div>
        <div className="absolute -bottom-6 -left-6 bg-white dark:bg-gray-800 px-5 py-3 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 opacity-0 group-hover/item3:opacity-100 transition-all duration-300 font-black text-sm text-gray-900 dark:text-white whitespace-nowrap z-50 transform translate-y-2 group-hover/item3:translate-y-0 flex items-center gap-2">
