...
import { ArrowLeft, ArrowRight, Calendar, Gift, Sparkles } from "lucide-react";
import { useState } from "react";
import { Link } from "react-router-dom";

export default function GiftIdeas() {
  const [page, setPage] = useState(0);
  const { data, loading, error } = useList(page, 9, "IDEAS");
  ...
  return (
    <div className="max-w-[1440px] mx-auto py-12 px-4 md:px-8">
      <SEO
        ...
        description="Сборник лучших идей для подарков на все случаи жизни."
      />
          ...
          <div className="w-full flex flex-col sm:flex-row justify-between items-end gap-6">
            <div className="space-y-2">
              ...
              <p className="text-gray-500 dark:text-gray-400 font-medium text-lg">
                Лучшие идеи здесь — от экспертов GiftoYou
              </p>
            </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {data?.map((post, index) => (
              ...
              >
                <Card className="glass-strong h-full flex flex-col rounded-[2.5rem] shadow-xl group-hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 overflow-hidden">
                  <div className="p-4 relative">
                    <div className="aspect-[16/10] w-full overflow-hidden rounded-[1.8rem] bg-gray-50 dark:bg-gray-800 relative">
                      {post.preview_image ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-main-theme/5">
                    ...
                    {/* Бейдж "Идея" */}
                    <div className="absolute top-8 left-8 bg-main-theme text-gray-900 font-black text-[10px] uppercase tracking-widest px-4 py-2 rounded-full flex items-center gap-1.5 shadow-lg">
                  ...
                  <div className="px-8 pt-2 pb-10 flex flex-col flex-1">
                    ...
                    <div className="mt-auto flex items-center text-main-theme font-black text-sm uppercase tracking-widest gap-2 opacity-0 group-hover:opacity-100 translate-x-[-10px] group-hover:translate-x-0 transition-all duration-500">
                      Смотреть идею <ArrowRight className="size-4" />
                    </div>
                  </div>
                </Card>
              </Link>
