...
export default function BlogList() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center space-y-6">
        ...
        <h2 className="text-3xl font-black text-gray-900">Не удалось загрузить статьи</h2>
        <p className="text-gray-500 max-w-md font-medium">{error}</p>
        <Button variant="outline" onClick={() => setPage(0)} className="h-14 px-8 border-2">
          Попробовать снова
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-[1440px] mx-auto py-12 px-4 md:px-8">
      <SEO title="Блог | GiftoYou" description="Статьи о выборе подарков для друзей и коллег." />

      <section className="space-y-12">
        <div className="flex flex-col gap-6">
          <Link to="/" className="group inline-flex items-center gap-2 text-sm text-gray-400 hover:text-main-theme transition-all font-bold">
            <ArrowLeft className="size-4 group-hover:-translate-x-1 transition-transform" /> На главную
          </Link>

          <div className="w-full flex flex-col sm:flex-row justify-between items-end gap-6">
            <div className="space-y-2">
              <h1 className="text-5xl md:text-6xl font-black text-gray-900 tracking-tight">Блог</h1>
              <p className="text-gray-500 font-medium text-lg">Полезные советы и идеи для идеальных праздников</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {data?.map((post, index) => (
              <Link key={post.id} to={`/blog/${post.slug}`} className="group h-full" data-reveal data-reveal-delay={index % 3}>
                <Card className="h-full flex flex-col rounded-[2.5rem] bg-white border-gray-100 shadow-xl group-hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-3 overflow-hidden border-none">
                  ...
                  <div className="px-8 pt-2 pb-10 flex flex-col flex-1">
                    <div className="flex items-center gap-2 text-xs text-main-theme font-black uppercase tracking-widest mb-4">
                      ...
                      <time dateTime={post.published_at}>{getFormatDate(post.published_at)}</time>
                    </div>
